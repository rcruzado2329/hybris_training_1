...
public class DefaultOrderFraudEmployeeNotificationEventListenerTest extends AbstractOrderManagementTest
{
	...
	public void setUp() throws Exception
	{
		...
		event = new OrderFraudEmployeeNotificationEvent(getOrder());
	...
	{
		assertThat(ticketDao.findTicketsByOrder(getOrder())).isEmpty();
		defaultOrderFraudEmployeeNotificationEventListener.onEvent(event);
		final List<CsTicketModel> findTicketsByOrder = ticketDao.findTicketsByOrder(getOrder());
		assertThat(findTicketsByOrder).hasSize(1);
		final CsTicketModel csTicketModel = findTicketsByOrder.get(0);
		Assert.assertEquals(getOrder(), csTicketModel.getOrder());
		Assert.assertEquals(getOrder().getUser(), csTicketModel.getCustomer());
