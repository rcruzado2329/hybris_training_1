...
public class CxSegmentMapperTest extends AbstractCxConsumptionLayerMapperTest
{
	...
	public void testMultipleSgmentWithPrefixWithSeparator()
	{
		...
		segmentMapper.setPrefix(prefix);
		segmentMapper.setSeparator(separator);
		...
		Assert.assertNotNull(target.getSegments());
		Assert.assertEquals(2, target.getSegments().size());
		assertAffinityForSegment(prefix + separator + SEGMENT_1, "10", target);
		assertAffinityForSegment(prefix + separator + SEGMENT_2, "10", target);
	}
	...
	protected Profile createProfile(final Map<String, Segment> segments)
	{
