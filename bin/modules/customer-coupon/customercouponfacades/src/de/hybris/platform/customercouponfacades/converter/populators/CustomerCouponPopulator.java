...
public class CustomerCouponPopulator implements Populator<CustomerCouponModel, CustomerCouponData>
{
	...
	public void populate(final CustomerCouponModel source, final CustomerCouponData target)
	{
		...
		if (isBindingAnyProduct(list))
		{
			target.setProductUrl(MessageFormat.format("/c/{0}?q=%3Arelevance%3AcustomerCouponCode%3A{1}&text=#",
					getSolrRootCategory(), source.getCouponId()));
			target.setSolrFacets(MessageFormat.format("\"%3Arelevance%3Acategory%3A{0}:customerCouponCode%3A{1}\"",
					getSolrRootCategory(), source.getCouponId()));
		}
		else
		{
			target.setProductUrl(MessageFormat.format("/c/{0}?q=%3Arelevance&text=#", getSolrRootCategory()));
			target.setSolrFacets(MessageFormat.format("%3Arelevance%3Acategory%3A{0}", getSolrRootCategory()));
		}
	...
	protected boolean isBindingAnyProduct(final List<PromotionSourceRuleModel> list)
	{
		return !list.isEmpty()
