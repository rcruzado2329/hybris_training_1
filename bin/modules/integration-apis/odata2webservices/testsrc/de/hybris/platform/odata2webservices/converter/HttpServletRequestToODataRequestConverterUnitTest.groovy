...
class HttpServletRequestToODataRequestConverterUnitTest extends Specification {
    ...
    @Test
    def 'converted headers are empty when original request does not have headers'() {
        given:
        def convertedReq = converter.convert createHttpServletRequest(HttpMethod.GET.name())

        expect:
        convertedReq.requestHeaders == [:]
        convertedReq.acceptHeaders == []
        convertedReq.acceptableLanguages == []
    }
