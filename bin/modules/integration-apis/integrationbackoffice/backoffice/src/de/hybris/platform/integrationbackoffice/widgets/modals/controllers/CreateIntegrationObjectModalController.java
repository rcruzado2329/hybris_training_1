...
public final class CreateIntegrationObjectModalController extends DefaultWidgetController {
    ...
    private List<IntegrationType> sortIntegrationTypes (List<IntegrationType> unsortededIntegrationTypes) {
        return unsortededIntegrationTypes.stream()
                .sorted(Comparator.comparing(IntegrationType::getCode))
                .collect(Collectors.toList());
    }

    private List<ComposedTypeModel> filterComposedTypeModels (String typeEnteredString, List<ComposedTypeModel> unfilteredComposedTypeModels) {
        return unfilteredComposedTypeModels
                .stream()
                .filter(p -> p.getCode().startsWith(typeEnteredString)).collect(Collectors.toList());
    }

    private List<IntegrationType> filterIntegrationTypes (String typeEnteredString, List<IntegrationType> unfilteredIntegrationTypes) {
    ...
    public void populateRootTypeComboBoxComboBox(String textInput) {
        ...
        List<ComposedTypeModel> sortedIntegrationObjectModels = sortComposedTypeModels(unsortedRootTypes);
