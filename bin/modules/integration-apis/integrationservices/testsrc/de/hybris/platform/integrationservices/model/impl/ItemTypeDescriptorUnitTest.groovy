...
class ItemTypeDescriptorUnitTest extends Specification {
    ...
    def "getAttributes() returns attributes defined in the item model"() {
        ...
        expect:
        descriptor.attributes.size() == itemModel.attributes.size()
        descriptor.attributes.collect({ it.getAttributeName() }).containsAll(['code', 'name'])
    }
    ...
    def "isInstance() returns true when the object's type is #condition"() {
        given:
        def itemModel = Stub(IntegrationObjectItemModel) {
            getType() >> composedType('UserGroup', composedType('Company'), composedType('Union'))
        }
        ...
        where:
        condition                      | obj
        'same as of descriptor'        | new UserGroupModel()
        'a sub-type of the descriptor' | new CompanyModel()
    }
